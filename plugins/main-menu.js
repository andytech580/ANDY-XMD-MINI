...
export default {
  ...
  run: async (client, m, args) => {
    try {
      const tiempo = moment.tz('Africa/Kampala').format('DD MMM YYYY')
      const tiempo2 = moment.tz('Africa/Kampala').format('hh:mm A')  
      ...
      const canalId = botSettings.id || '120363421568891511@newsletter'  
      const canalName = botSettings.nameid || '𝑨𝑵𝑫𝒀-𝑿𝑴𝑫 𝑴𝑰𝑵𝑰'  
      ...
      const categoryNames = {  
        ...
        religion:'🛐 RELIGION COMMANDS',
        tools:'⚒️TOOLS COMMANDS',
  ...
  if (selectedId === 'menu_stats') {
    ...
    await client.sendInteractiveButtons(m.chat, {
      ...
      interactiveButtons: [
        ...
        {
          name: 'cta_url',
          buttonParamsJson: JSON.stringify({ display_text: '🐙 GitHub Repository', url: global.bot.github || 'https://github.com/andytech580/ANDY-MINI' })
        }
      ]
    })
