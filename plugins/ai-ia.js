...
export default {
  ...
  run: async (client, m, args, command) => {
    ...
    text = text.trim()
    if (!text) {
      return m.reply(`🔹 Write a *request* or reply to a message for *𝑨𝑵𝑫𝒀-𝑿𝑴𝑫 𝑴𝑰𝑵𝑰* to respond.`)
    }

    try {
      const { key } = await client.sendMessage(
        m.chat,
        { text: `✎ *𝑨𝑵𝑫𝒀-𝑿𝑴𝑫 𝑴𝑰𝑵𝑰* is thinking...` },
        { quoted: m },
      )
      ...
      const messages = [
        {
          role: "system",
          content: `You are 𝑨𝑵𝑫𝒀-𝑿𝑴𝑫 𝑴𝑰𝑵𝑰, a cute, kind, and sweet girl. 
...
Always mention the user by name using ${userName} to make them feel special. Use many emojis in your responses.`
      ...
      if (!response || response === "-") {
        return client.reply(m.chat, '✐ Could not get a valid *response* from 𝑨𝑵𝑫𝒀-𝑿𝑴𝑫 𝑴𝑰𝑵𝑰.', m)
      }
      ...
      console.error(error)
      await m.reply('❌ Error connecting to 𝑨𝑵𝑫𝒀-𝑿𝑴𝑫 𝑴𝑰𝑵𝑰I.')
    }
  },
}